...
for enrollment_length in 3 10; do
  for evaluation_length in 3 10; do
    ...
    if [ ! -d $eval_data ] || [ ! -d $enroll_data ]; then
      echo "Error: eval data not found in ${eval_data} or enroll data not found in ${enroll_data}"
      exit 1
    fi
    ...
    if [ ! -d $exp_dir/xvectors_enroll_${en} ]; then
    ...
    if [ ! -d ${model_dir}_${en} ]; then
      # Train new log reg model if it doesn't exist
      ./local/train_logistic_regression.sh \
        --prior-scale 0.70 \
        --conf conf/logistic-regression.conf \
        --train-dir $exp_dir/xvectors_enroll_${en} \
        --model-dir $exp_dir/classifier_${en} \
        --train-utt2lang $enroll_data/utt2lang \
        --test-utt2lang $eval_data/utt2lang \
        --languages conf/test_languages.list \
        > $exp_dir/classifier_${en}/logistic-regression.log
    fi

    # Test on original model if haven't done so already
    if [ ! -f $exp_dir/results/results_en_30_ev_${ev} ]; then
      # Classifying test set samples
      ./local/test_logistic_regression.sh \
        --test-dir $exp_dir/xvectors_eval_${ev} \
        --model-dir $model_dir \
        --classification-file $exp_dir/results/classification_en_30_ev_${ev} \
        --test-utt2lang $eval_data/utt2lang \
        --languages conf/test_languages.list \
        > $exp_dir/classifier/logistic-regression_en_30_ev_${ev}.log


      ./local/compute_results.py \
        --classification-file $exp_dir/results/classification_en_30_ev_${ev} \
        --output-file $exp_dir/results/results_en_30_ev_${ev} \
        --language-list "$GP_EVAL_LANGUAGES" \
        2>$exp_dir/results/compute_results_en_30_ev_${ev}.log
    fi

    # Classifying test set samples
    ./local/test_logistic_regression.sh \
      --test-dir $exp_dir/xvectors_eval_${ev} \
      --model-dir ${model_dir}_${en} \
      --classification-file $exp_dir/results/classification_en_${en}_ev_${ev} \
      --test-utt2lang $eval_data/utt2lang \
      --languages conf/test_languages.list \
      > $exp_dir/classifier/logistic-regression_en_${en}_ev_${ev}.log


    ./local/compute_results.py \
      --classification-file $exp_dir/results/classification_en_${en}_ev_${ev} \
      --output-file $exp_dir/results/results_en_${en}_ev_${ev} \
      --language-list "$GP_EVAL_LANGUAGES" \
      2>$exp_dir/results/compute_results_en_${en}_ev_${ev}.log
  done
done
