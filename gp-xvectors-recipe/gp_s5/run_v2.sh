...
# Will exit if any nonzero status returned
set -e
mfccdir=`pwd`/mfcc
vaddir=`pwd`/mfcc
nnetdir=exp/xvector_nnet_1a

#!!! Check before running and replace as appropriate
DATADIR=/afs/inf.ed.ac.uk/user/s15/s1531206/gp_data/data
...
if [ $stage -le 0 ]; then
  ...
  local/gp_data_prep_new.sh --config-dir=$PWD/conf --corpus-dir=$GP_CORPUS --languages="$GP_LANGUAGES" --data-dir=$DATADIR || exit 1;
...
if [ $stage -le 1 ]; then
  ...
  for name in train eval; do
    steps/make_mfcc.sh --write-utt2num-frames true --mfcc-config conf/mfcc.conf --nj 40 --cmd "$train_cmd" \
      $DATADIR/${name} exp/make_mfcc $mfccdir
    utils/fix_data_dir.sh $DATADIR/${name}
    sid/compute_vad_decision.sh --nj 40 --cmd "$train_cmd" \
      $DATADIR/${name} exp/make_vad $vaddir
    utils/fix_data_dir.sh $DATADIR/${name}
  done
